    ...
    .factory('satellizer.shared', [
      ...
      function($q, $window, $location, config, storage) {
        ...
        shared.getUser = function() {
            return JSON.parse(storage.get(config.tokenPrefix+'_user'));
        };
        ...
        shared.setToken = function(response, redirect) {
          ...
          storage.set(config.tokenPrefix + '_user', JSON.stringify(response.data['user']));
        ...
        shared.removeToken = function() {
          ...
          storage.remove(config.tokenPrefix+'_user');
        };
